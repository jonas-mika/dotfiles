...
`);
  ...
  disposable = import_coc24.commands.registerCommand("pyright.version", () => {
    const json = (0, import_path5.join)(context.extensionPath, "node_modules", "pyright", "package.json");
    const packageJSON = JSON.parse((0, import_fs.readFileSync)(json, "utf8"));
    import_coc24.window.showMessage(`Pyright version ${packageJSON.version}`);
  });
  context.subscriptions.push(disposable);
...
var ImportCompletionProvider = class {
  async provideCompletionItems(document, position, token, context) {
    ...
    const source = import_coc24.sources.getSource("pyright-1");
    if (!source)
      return [];
